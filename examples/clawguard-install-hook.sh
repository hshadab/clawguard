...
#
# Modes:
#   1. Hosted API mode: set CLAWGUARD_API_URL to a running ClawGuard server
#      CLAWGUARD_API_URL=http://localhost:8080 ./examples/clawguard-install-hook.sh weather-helper
#
#   2. Local binary mode (default): requires clawguard + clawhub in PATH
#      Requirements:
...
CLAWGUARD_API_URL="${CLAWGUARD_API_URL:-}"

# ---- Hosted API mode ----
if [ -n "$CLAWGUARD_API_URL" ]; then
    echo "Using hosted ClawGuard API: ${CLAWGUARD_API_URL}"
    ...
    SCAN_OUTPUT=$(curl -s -X POST \
        ...
        -d "$REQUEST_BODY" \
        "${CLAWGUARD_API_URL}/api/v1/evaluate/name") || {
        echo "WARNING: Failed to reach ClawGuard API. Proceeding with caution."
        exit 0
    }
    ...
    if [ -z "$SUCCESS" ]; then
        ...
        echo "WARNING: ClawGuard API error: ${ERROR:-unknown}. Proceeding with caution."
        exit 0
    fi

    # Parse the decision from the receipt
